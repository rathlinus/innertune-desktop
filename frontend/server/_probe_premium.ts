...
//
// The decorator has a distinctive shape — `NAME=function(url,key,val){url=new
// URLCTOR(url,!0);url.set("alr","yes");…}` — so we match that directly rather than
// grabbing the first `("alr","yes")` in the file. Newer players (e.g. 2574220e)
// also stamp `("alr","yes")` inside a *segment*-URL builder that sorts earlier in
// base.js and, being a `g.NAME` property that takes a segment object (not a URL
// string), is neither the signer nor callable by bare name through the portal —
// picking it left the decorator drive returning undefined and every HQ/authed
// resolve falling through to the static n-descramble, which then 502'd. The
// lookbehind rejects a `.`-prefixed property so we only take a standalone
// (portal-callable) closure name.
function decoratorName(assets: PlayerAssets): string | null {
  ...
  let name: string | null = sig ? sig[1] : null;
  // Fallback for older families where the first `("alr","yes")` sits in the
  // decorator itself: take the nearest preceding `NAME=function(`.
  if (!name) {
    const at = js.search(/"alr"\s*,\s*"yes"/);
    const head =
      at < 0 ? null : [...js.slice(0, at).matchAll(/\b([A-Za-z0-9$_]+)=function\(/g)].pop();
