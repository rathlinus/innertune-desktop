...
// set numeric high and load a fresh track
console.log("setNum3+load:", await ev(c.call,`(function(){var p=document.getElementById('movie_player');try{p.setUserAudioQualitySetting(3);}catch(e){} var v=document.querySelector('video');if(v)v.muted=true; try{p.loadVideoById&&p.loadVideoById('dQw4w9WgXcQ');}catch(e){return 'loaderr '+e} return 'loaded';})()`));
await wait(10000);
console.log("after-load audioState:", await ev(c.call,`(function(){var p=document.getElementById('movie_player');return JSON.stringify({user:p.getUserAudioQualitySetting(),paygated:p.getPaygatedAudioQualityData(),hqa:p.hasHqaAudioTrack(),avail:p.getAvailableQualityData&&p.getAvailableQualityData()});})()`).then(x=>String(x).slice(0,400)));
...
console.log("itags fetched:", Object.keys(byItag).join(",")||"none");
